...
import { AuditDashboardDto, HistoricalSessionDto, SessionAuditDetailDto } from '@/lib/types/api';
...
function Podium({
  ...
  entries: AuditDashboardDto['generalRanking'];
      ...
      {ordered.map(({ entry, height, medal, label }) => (
        <div key={`${entry.sessionId}-${entry.teamId}`} className="flex w-20 flex-col items-center sm:w-24">
          ...
          >
            <span
              className={cn(
                'mb-2 inline-flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-bold shadow',
                medal,
              )}
            >
              {label}
...
export function AuditHistoryView() {
  ...
  useEffect(() => {
    void Promise.all([
      operatorSessionService.getHistoricalSessions(),
      operatorSessionService.getAuditDashboard(),
    ])
    ...
    if (operatorId) {
      void operatorSessionService
        .getAssignedMissions(operatorId)
        .then((missions) => {
          const map: Record<string, string> = {};
          for (const mission of missions) map[mission.missionId] = mission.title;
  ...
  const operatorChartData = useMemo(
    () =>
      (dashboard?.topOperators ?? []).map((item) => ({
        name: shortLabel(truncateId(item.operatorId), 12),
        fullName: truncateId(item.operatorId),
        sessions: item.sessionCount,
      })),
    [dashboard],
  );
    ...
    return (
      <div>
        <div className="mb-8 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Auditoría de sesión</h1>
    ...
    <div>
      <div className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-card via-card to-muted/40 p-6 sm:p-8">
        ...
        <div className="relative">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground">Tu rendimiento</h1>
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Ranking, misiones y sesiones de tu operación en un vistazo.
          </p>
        </div>
      </div>
      ...
      {dashboard && (
        <>
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            />
            <KpiCard
              label="Misiones jugadas"
              value={dashboard.topMissions.length}
              hint="En tus sesiones"
              icon={MedalIcon}
              accent="bg-sky-500"
            />
            <KpiCard
              label="Equipos en ranking"
              value={dashboard.generalRanking.length}
              hint="Top performances"
              icon={UsersIcon}
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
            ...
            <section className="rounded-xl border border-border bg-card p-5 shadow-sm lg:col-span-1">
              <h2 className="text-sm font-medium text-foreground">Sesiones por operador</h2>
              <p className="mb-4 text-xs text-muted-foreground">Volumen de tus sesiones cerradas</p>
      ...
      <div className="mt-4 space-y-3">
        {sessions.map((session) => (
          <button
            ...
            type="button"
            className="w-full rounded-xl border border-border bg-card p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-foreground/20 hover:shadow-md"
            onClick={() => void openDetail(session.sessionId)}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-medium text-foreground">{missionLabel(session.missionId)}</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  Sesión {truncateId(session.sessionId)} · {new Date(session.startedAtUtc).toLocaleString()}
                </p>
              </div>
              <span
                className={cn(
