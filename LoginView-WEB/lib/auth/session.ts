...
export type UmbralRole = 'admin' | 'operator' | 'player'
...
export function buildRedirectUrl(baseUrl: string, session: AuthSession): string {
  ...
  return `${baseUrl.replace(/\/+$/, '')}/#auth=${encodeURIComponent(params.toString())}`
}
